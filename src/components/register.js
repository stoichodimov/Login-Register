...
function Register() {
	...
	function handleChange(e) {
		...
		switch (name) {
			...
			case 'lastName':
				if (minMaxLength(value, 3)) {
					currentFormErrors[name] = `Last Name should have minimum 3 characters`;
				} else {
					delete currentFormErrors[name];
				}
				break;
			case 'email':
				if (!value || validEmail(value)) {
					...
					userExists(value).then((result) => {
						if (result) {
							currentFormErrors[name] = 'The email is already registered. Please use a different email.';
						} else {
							delete currentFormErrors[name];
						}
					});
			...
			case 'password':
				if (minMaxLength(value, 8)) {
					...
					if (user.confirmpassword) {
						validateConfirmPassword(value, user.confirmpassword, currentFormErrors);
					}
				}
				break;
			case 'confirmpassword':
				let valid = validateConfirmPassword(user.password, value, currentFormErrors);
				if (valid) {
			...
			<form onSubmit={submit} noValidate>
				<div className="row">
					<div className="col-md-6">
						...
						<input
							className={formErrors && formErrors.firstName ? 'form-control error' : 'form-control'}
				...
				<div className="mb-3">
					...
					<input
						className={formErrors && formErrors.password ? 'form-control error' : 'form-control'}
				...
				<div className="mb-3">
					...
					<input
						className={formErrors && formErrors.confirmpassword ? 'form-control error' : 'form-control'}
						placeholder="Password"
						type="password"
				...
				<div className="mb-3">
					<button type="submit" disabled={Object.entries(formErrors || {}).length > 0}>
						Create Account
					</button>
				</div>
			</form>
